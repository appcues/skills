...
Eight portable Agent Skills (`skills/<name>/SKILL.md`) for reporting on an Appcues account, plus thin per-runtime manifests at the root so one tree installs into Claude Code, Codex, Hermes Agent, and OpenClaw. The layout mirrors anthropics/skills: everything of substance lives in `skills/`, and every manifest points at that same directory. There is no build, no dependency install, and no test suite; the checks below are the whole verification story.
...
| File | Read by |
|---|---|
| `plugin.json` | Hermes (`hermes plugins install`), Codex (portable root manifest), OpenClaw (Agent Plugins bundle marker) |
...
Versions are duplicated: `plugin.json` and `.codex-plugin/plugin.json` carry the plugin version, `.claude-plugin/plugin.json` has its own. Bump them together.
